...
for index_height = 1:4000
    ...
    time_ideal = time_table_1{index_height, 6};
    pixel_ideal = get_y_value(time_ideal, x_ideal, y_ideal);

    time_expect_1 = time_table_1{index_height, 7}+0.0011;
    pixel_expect_1 = get_y_value(time_expect_1, x_ideal, y_ideal);

    time_expect_2 = time_table_2{index_height, 7}+0.0011;
    ...
    time_start_actual_1(index_height, :) = [seconds(current_diff), value_1(index_height), pixel_ideal, pixel_expect_1, seconds(current_diff_1), seconds(current_diff_1)-seconds(current_diff)];

    current_diff = time_table_2{index_height, 3} - time_table_2{1, 2};
    current_diff.Format = 'mm:ss.SSSSSS';
    time_start_actual_2(index_height, :) = [seconds(current_diff), value_2(index_height), pixel_ideal, pixel_expect_2, seconds(current_diff_2), seconds(current_diff_2)-seconds(current_diff)];
